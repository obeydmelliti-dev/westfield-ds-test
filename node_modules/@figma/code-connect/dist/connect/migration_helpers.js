...
exports.prepareMigratedTemplateBody = prepareMigratedTemplateBody;
...
function prepareMigratedTemplateBody(doc, includeProps = false, useTypeScript) {
...
const migrateV1TemplateToV2 = (template) => {
    ...
    migrated = migrated.replace(/\.__findChildWithCriteria__\(\{\s*name:\s*'([^']+)',\s*type:\s*"TEXT"\s*\}\)\.__render__\(\)/g, ".findText('$1').__FCC_TEXT_RENDER__()");
    migrated = migrated.replace(/\.__findChildWithCriteria__\(\{\s*type:\s*"TEXT",\s*name:\s*'([^']+)'\s*\}\)\.__render__\(\)/g, ".findText('$1').__FCC_TEXT_RENDER__()");
